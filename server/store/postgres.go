...
func (s *PostgresStore) queryRowContext(ctx context.Context, query string, args ...any) *sql.Row {
	return s.db.QueryRowContext(ctx, rebindPostgresPlaceholders(query), args...)
}

func txExecContextPG(ctx context.Context, tx *sql.Tx, query string, args ...any) (sql.Result, error) {
	return tx.ExecContext(ctx, rebindPostgresPlaceholders(query), args...)
}

func txQueryRowContextPG(ctx context.Context, tx *sql.Tx, query string, args ...any) *sql.Row {
	return tx.QueryRowContext(ctx, rebindPostgresPlaceholders(query), args...)
}
...
func (s *PostgresStore) DeletePushSubscriptionByEndpoint(ctx context.Context, endpoint string) error {
	const query = `DELETE FROM push_subscriptions WHERE endpoint = ?`
	return deletePushSubscription(ctx, s.execContext, query, endpoint)
}

func (s *PostgresStore) DeletePushSubscriptionByEndpointForUser(ctx context.Context, userID, endpoint string) error {
	const query = `DELETE FROM push_subscriptions WHERE user_id = ? AND endpoint = ?`
	return deletePushSubscription(ctx, s.execContext, query, userID, endpoint)
}

func (s *PostgresStore) ListPushSubscriptionsByUser(ctx context.Context, userID string) ([]models.PushSubscription, error) {
	const query = `
...
func (s *PostgresStore) SearchUsersByUsername(ctx context.Context, params models.SearchUsersParams) ([]models.User, error) {
	limit := params.Limit
	if limit <= 0 {
		limit = 20
	}
	...
	rows, err := s.queryContext(ctx, query, like, limit)
	if err != nil {
...
func (s *PostgresStore) ListConversationsByUser(ctx context.Context, params models.ListUserConversationsParams) ([]models.ConversationSummary, error) {
	limit := params.Limit
	if limit <= 0 {
		limit = 50
	}
...
func (s *PostgresStore) GetOrCreateDirectConversation(ctx context.Context, params models.GetOrCreateDirectConversationParams) (models.Conversation, error) {
	...
	if err != nil {
		...
		conversation, getErr := getConversationByParticipantsTxPG(ctx, tx, participantA, participantB)
		if getErr == nil {
			if err := tx.Commit(); err != nil {
	...
	conversation, err = getConversationByIDTxPG(ctx, tx, params.ConversationID)
	if err != nil {
		return models.Conversation{}, err
	}
...
func (s *PostgresStore) ListMessagesByConversation(ctx context.Context, params models.ListConversationMessagesParams) ([]models.MessageDetails, error) {
	...
	if params.BeforeMessageID != nil && strings.TrimSpace(*params.BeforeMessageID) != "" {
		...
		`
		if err := s.queryRowContext(ctx, beforeQuery, beforeID, params.ConversationID).Scan(&beforeCreatedAtText); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
...
func (s *PostgresStore) UpdateMessage(ctx context.Context, params models.UpdateMessageParams) (models.Message, error) {
	const query = `
		UPDATE messages
...
func (s *PostgresStore) ToggleMessageReaction(ctx context.Context, params models.ToggleMessageReactionParams) (models.ToggleReactionResult, error) {
	message, err := s.getMessageByID(ctx, params.MessageID)
	if err != nil {
	...
	existing, err := getReactionByMessageUserEmojiTxPG(ctx, tx, params.MessageID, params.ActorUserID, emoji)
	if err == nil {
		if err := deleteReactionByIDTxPG(ctx, tx, existing.ID); err != nil {
			return models.ToggleReactionResult{}, err
		}
		if err := tx.Commit(); err != nil {
	...
	addedReaction, err := getReactionByIDTxPG(ctx, tx, reactionID)
	if err != nil {
		return models.ToggleReactionResult{}, err
	}
...
func (s *PostgresStore) RemoveMessageReaction(ctx context.Context, params models.RemoveMessageReactionParams) (models.Reaction, error) {
	message, err := s.getMessageByID(ctx, params.MessageID)
	if err != nil {
	...
	reaction, err := getReactionByMessageUserEmojiTxPG(ctx, tx, params.MessageID, params.ActorUserID, emoji)
	if err != nil {
		return models.Reaction{}, err
	}
	if err := deleteReactionByIDTxPG(ctx, tx, reaction.ID); err != nil {
		return models.Reaction{}, err
	}
...
func (s *PostgresStore) configure(ctx context.Context) error {
...
func (s *PostgresStore) ensureConversationParticipant(ctx context.Context, conversationID, userID string) error {
...
func getReactionByIDTxPG(ctx context.Context, tx *sql.Tx, reactionID string) (models.Reaction, error) {
	return getReactionByQueryRowPG(txQueryRowContextPG(ctx, tx, `
...
func getReactionByMessageUserEmojiTxPG(ctx context.Context, tx *sql.Tx, messageID, userID, emoji string) (models.Reaction, error) {
	return getReactionByQueryRowPG(txQueryRowContextPG(ctx, tx, `
		SELECT id, message_id, user_id, emoji, created_at
		FROM reactions
...
func deleteReactionByIDTxPG(ctx context.Context, tx *sql.Tx, reactionID string) error {
	res, err := txExecContextPG(ctx, tx, `DELETE FROM reactions WHERE id = ?`, reactionID)
	if err != nil {
...
func (s *PostgresStore) getPushSubscriptionByEndpoint(ctx context.Context, endpoint string) (models.PushSubscription, error) {
	const query = `
		SELECT id, user_id, endpoint, p256dh, auth, user_agent, created_at, updated_at
		FROM push_subscriptions
		WHERE endpoint = ?
	...
	row := s.queryRowContext(ctx, query, endpoint)
