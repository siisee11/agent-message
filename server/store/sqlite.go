...
func (s *SQLiteStore) UpdatePasswordHash(ctx context.Context, params models.UpdatePasswordHashParams) (models.User, error) {
	const query = `
		UPDATE users
		SET password_hash = ?
		WHERE id = ?
	`
	res, err := s.db.ExecContext(ctx, query, params.PasswordHash, params.UserID)
	if err != nil {
