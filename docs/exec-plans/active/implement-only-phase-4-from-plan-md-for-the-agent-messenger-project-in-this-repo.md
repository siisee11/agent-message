...
## Milestones
- [ ] M1. Scaffold `web/` with Vite React + TypeScript, configure baseline project structure, and install minimal dependencies for routing/data-fetching/styling (status: not started)
...
- Choose the smallest reliable dependency set for Phase 4:
  ...
  - Data fetching/cache: one lightweight query layer (React Query or SWR)
  - Styling: minimal baseline (CSS modules or lightweight global CSS), no Phase 5 UI system build-out
...
- Confirm preferred data-fetching library choice if both are acceptable (`@tanstack/react-query` vs `swr`), using minimal complexity as the tie-breaker.
